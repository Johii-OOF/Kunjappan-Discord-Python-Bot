...
async def ch_pr():
    ...
    statuses = [
        "joe mama", "with ur mum",
        f"on {len(client.guilds)} servers | da help", "kanye songs",
        "with no one", "mods", "Amoongus", "tomato", "with your thantha"
    ]
...
async def snipe(ctx):
    ...
    try:
        ...
        em.set_footer(
            text=
            f"That message was sent by {snipe_message_author[channel.id]}, nee menden anallo"
        )
        await ctx.send(embed=em)
    except KeyError:
        await ctx.send(
            f"fuck off, No deleted messages found in #{channel.name} <a:NoOoOo:907224828652707900> epic fail L 👎",
            delete_after=3)
...
async def oruhelp(ctx):
  reply = await ctx.send("https://images-ext-1.discordapp.net/external/5RaeROFKDy2vtlOlWbTLK-TmA3y2yYkjfl894whRnaI/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/778553831114014721/c7878349efc0eeabc11a609e84e425f8.webp?width=669&height=669")
  await asyncio.sleep(1)
  await reply.edit(content="sure babe, \n here's the link: https://johii.carrd.co/")
...
@client.command(case_insensitive=False,
                help='This command posts the news link')
@commands.cooldown(1, 5, commands.BucketType.user)
async def news(ctx):
    await ctx.send("https://youtu.be/rWTZyuy6QV4", delete_after=10)
...
@client.command(aliases=['av'],
                name='avatar',
                help='Shows the avatar of the user')
@commands.cooldown(1, 10, commands.BucketType.user)
...
async def _8ball(ctx, *, question):
    responses = [
        'ada 👍', 'no 😳', 'idk man', 'brr i forgor 💀', 'yes', 'fuck off',
        'my sources say no', 'its a no from me', 'onnu poyeda 😒',
        'ayee injathi vanam', 'yesnt', 'fuck off 😑', 'Huh', 'Tf', 'myre', '👌',
        'am not that guy', '😑',
        'https://cdn.discordapp.com/emojis/895312765613391872.png?size=128',
        'https://cdn.discordapp.com/attachments/775769580895338508/936544015795503114/Banana_council_disaproves_of_this.mp4',
        'https://tenor.com/view/sopyues-spongebob-spongebob-meme-twerking-twerk-gif-20539152',
        'https://c.tenor.com/LSdPTJ_Uki4AAAAM/haram-heisenberg.gif', 'man',
        'ah',
        'https://cdn.discordapp.com/attachments/775769580895338508/948561154031104012/Tyler_The_Creator_-_5__480_X_526_.mp4',
        'https://cdn.discordapp.com/attachments/775769580895338508/948557545923051580/Tyler_The_Creator_-_3__240_X_266_.mp4'
    ]
...
async def meme(ctx):
    async with aiohttp.ClientSession() as cs:
        async with cs.get('https://www.reddit.com/r/memes/hot.json') as r:
            res = await r.json()
            titleurl = res['data']['children'][random.randint(
                0, 25)]["data"]["title"]
            title = (titleurl)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        msg = 'Enich podey, try after {:.2f}s, <a:NotFunnyBlink:906927061296349196> ||<a:uh:906917859899367464> you are annoying <a:uh:906917859899367464>||'.format(
            error.retry_after)
        await ctx.reply(msg)
